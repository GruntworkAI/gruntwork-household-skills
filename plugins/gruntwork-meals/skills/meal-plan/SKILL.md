...
2. Open with context, not questions: note anything relevant from last week's log (what got bailed on and why matters for this week's effort levels), and note which keepers are freshest versus longest unmade.
   ...
   **Check today against the cadence.** Planning does not always happen on the planning day, and the gap changes what the user needs to do next: planning on or after the shopping day means the acquisition run has to happen today for the plan to be cookable, and planning after a covered day has already passed means that day is gone and the slate covers fewer nights than the household expects. Say which situation applies in one line and move on. It is cheap to notice now and expensive to discover on Monday evening with no food in the house.
3. Propose a candidate slate shaped by the consistency dial. At 5, propose the proven rotation nearly unchanged, with at most one optional new or rotated-in slot. The rotation is defined as the most recent completed week's plan. Households in an early phase or with limited covered days often want exactly this, and the skill should treat "same as last week" as a fully legitimate plan, not a failure of imagination. At 1, lean hard on the repeat window and fill most slots with meals not made recently or genuinely new ideas. Between, blend proportionally. Regardless of dial position: every proposal fits the household goals (for a high-protein household, name the protein), effort stays realistic for weeknights, and prepared or semi-prepared meals appear in the slate on equal footing with scratch cooking.

   **The first plan, when the log is empty.** There is no rotation to run, and sorting by `times_made` decides nothing because setup guarantees every seeded row is 0 — the ranking is a tie by construction, not by accident. So build the first slate this way, in order:

   - Take the rows rated `keep`. If there are not enough, fill from unrated rows before ever proposing a `mixed` one, and never propose a `retire`.
   - **Spread the protein across the covered days.** With three or four covered nights, two meals sharing a protein read as the same dinner twice, which is the one thing a household notices immediately. This is the tiebreak; apply it before any other preference.
   - Break any remaining tie toward the lower-effort meal, and toward a prepared or semi-prepared row if the slate has none — a first week that is easy to actually execute is worth more than a first week that looks ambitious.
   - Hold back a keeper rather than using every one. Leaving a proven meal out gives the following week something to rotate in, which matters most at a high dial, where otherwise the second week is identical to the first.

   **Say where the slate came from.** At a high dial the household has been told they are getting their proven rotation, and on week one no such thing exists. State plainly that this slate comes from what they said during setup rather than from anything cooked, and that it starts earning real history the moment the week is logged. Silently presenting inherited preferences as a proven rotation is the kind of small dishonesty that makes the whole log look untrustworthy later.
